...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
    ...
    H, W, C = im_size
    block_size = (H//2, W//2)
    im_left = torch.zeros((H, W, 3)) # placeholder, not actual size
    im_right = torch.zeros((H, W, 3)) # placeholder, not actual size
    ############################################################################
    # Student code begin
    ############################################################################
    print("input disparity:", disparity)
    left_image = torch.randint(0, 2, (H, W))
    right_image = torch.clone(left_image)
#     torch.set_printoptions(threshold=10_000)

#     print("right before shifting")
#     print(right_image)
    
    # measurement on what to crop
    tl = (block_size[0] - block_size[0]//2, block_size[1] - block_size[1]//2)
    br = (block_size[0] + block_size[0]//2, block_size[1] + block_size[1]//2)    
    # crop the disparity block
    crop_block = right_image[tl[0]:br[0]+1,tl[1]:br[1]+1]
    
#     print("crop_block", crop_block.size(), "should match the block_size", block_size)
    # shift the crop_block to the left horizontally by disparity pixels
    right_image[tl[0]:br[0]+1, tl[1]-disparity:br[1]-disparity+1] = crop_block
    
    # get the indices that need to refill after shifting
    refill_indices_tl = (tl[0], br[1]-disparity+1)
    # refill the image with the random values
#     refill = torch.full((disparity, tr[1]+1-tl[1]), 4) # for debugging purpose
    refill = torch.randint(0, 2, (br[0]+1-tl[0], disparity))
    right_image[tl[0]:br[0]+1,br[1]-disparity+1:br[1]+1] = refill
#     print("refill", refill.size())

#     print("right after shifting")
#     print(right_image)
    
    # results
    im_left = left_image.unsqueeze(2).expand(H, W, 3)
#     print("left_size", im_left.size())
    im_right = right_image.unsqueeze(2).expand(H, W, 3)
#     print("right_size", im_right.size())
